...
class StealthConn(object):
    ...
    def send(self, data):
        # Only after the encryption is done, the client will 
        # send encrypted msg
        ...
        # If self.cipher is not set, then it means the "send" 
        # is just to send g_a
        else:
            encrypted_data = data
    ...
    def recv(self):
        ...
        # Again, only after the encryption is done, 
        # the server will do decryption
        # Otherwise, it will just receive the original, 
        # as used to receive g_a
        if self.cipher:
            ...
            if bytes(encrypted_msg_hash,'ascii') == hash:
                ...
                # Take out the seperate the initial vector 
                # and encrypted msg
                ...
                # At this point, the msg consists of the data 
                # to send, and the random number to prevent 
                # replay attack
